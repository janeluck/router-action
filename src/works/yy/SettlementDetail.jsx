...
import {Button, Modal, Input, Tabs, Checkbox, message, Spin, Popover} from 'antd'
...
import {save} from 'src/common/redux/modules/billing/mix'
...
export class RightContent extends Component {
  ...
  handleSave = () => {
    ...
    //let presellMoney = this.receipts
    // 判断是否为预订状态， 设定预订金额
        ...
        return new Promise(function (resolve, reject) {
          refundorcancel({
            ...
            if (json.code == 200) {
              ...
              cbError(`${paymode.name}退款失败, 金额为${paymode.value}, ${json.message}`)
              reject(json)
            }
...
// 多种支付方式是同步的
      ...
      if (value && value != '') {
        controls.push(
          <div key={ele.dataIndex}>
            <span>{ele.showItem}</span><p>{value}</p>
          </div>
        )
      }
    }, this);
  ...
  toggleDelZero = (e) => {
    let lineConnection = this.props.lineConnection;
    if(!lineConnection){
  ...
  renderDelZero = (data) => {
    ...
    return (
      <div className="change-count">
        ...
        <span className='render-zero'><Checkbox onChange={this.toggleDelZero} checked={isDefaultValue && lineConnection}/></span>
      </div>
...
// 是否显示上一单信息
  ...
  render() {
    ...
    for (let attr in money) {
      ...
      preferentials.push(<div key={attr} className="billing-touch-preferential">
    ...
    const quickPayLength = (quickPay || []).slice(0, 2).length



    const content = this.isShowLastBill(lastBill, billingStatus, bRepair, bHang, products, userInfoStatus) ? (<div>
      <div className="billing-touch-s-details">
      ...
      <div className="bill-juc"></div>
      <div className="billing-touch-title-count">
        <div className="billing-touch-line"></div>
        <h1 className="billing-touch-title">结算明细</h1>
      </div>


      <div className="billing-touch-conts">
        <div className="billing-touch-list">
          <div className="billing-touch-list-money">
            <div className="lf-name">商品金额</div>
            <div className="fr-money">{this.total}</div>
          </div>
    ...
    return (
      ...
      <div>
        ...
        <Popover content={content} title={null} trigger="click"
                 placement="topLeft"
                 overlayClassName="bill-touch-Settlement-popup"
                 visible={this.state.popVisible} onVisibleChange={this.handlepopVisibleChange}
        >
          <div className="touch-bottom-left-Settlement">
            <div className="bill-Settlement">
              <i className="anticon anticon-jiesuanmingxi"></i>
              <span
                className="fr">{getFixedNumber(money.TotalQuantity.value, 'quantitydecimal') > 99 ? '99+' : getFixedNumber(money.TotalQuantity.value, 'quantitydecimal')}</span>
            </div>
            <div className="billing-touch-btns">
              {!productsIsEmpty && <div className='billing-touch-conts'>


                {/*交货状态下显示'已收款'*/}
                {(billingStatus === 'Shipment' || billingStatus === 'OnlineBill' ) &&
                <div className="clearfix billing-touch-receivables"><span>已收款</span><span
                  className="billing-touch-paid">{getFixedNumber(money.Deposit.value)}</span></div>}
                <div className={classnames('money', {
                  "billing-touch-paid-money": billingStatus === 'Shipment' || billingStatus === 'OnlineBill'
                })}><span>应收</span><span className="fr-money-ys ">{this.receivable}</span></div>
              </div>}
        ...
        <div className={classnames('billing-touch-js-zfb', 'clearfix', `billing-touch-quickPayTotal${quickPayLength}`)}>

          <Button
            className={classnames(`billing-touch-button-defaultSettle${quickPayLength}`)}
            key="settle" type="pay" onClick={()=>{
              if(!this.props.lineConnection){
                cb.utils.alert('当前网络不可用，不能使用此功能', 'error')
                return
              }
              paymodeActions.defaultSettle()
            }}
            disabled={productsIsEmpty}>{settle.name}</Button>

          {_.map((quickPay || []).slice(0, 3), item => {
            //快捷支付方式
            return (
              <Button

                key={item.paymethodId} onClick={() => {
                  if(!this.props.lineConnection && item.paymentType !== 1){
                    cb.utils.alert('当前网络不可用，不能使用此功能', 'error')
                    return
                  }
                ...
                className={classnames("cash-Settlement", `billing-touch-button-quickPay${quickPayLength}`)}
...
function mapStateToProps(state) {
  return {
    ...
    lineConnection: state.offLine.get('lineConnection')
  }
}
